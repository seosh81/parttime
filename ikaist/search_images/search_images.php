...
<?php
	...
	if (!empty($q)) {
		...
		if (count($image_array) == 0 || $RandomPickup == "0") {
			echo $result_html;
		} else if ($RandomPickup == "1") {
			$random_number = rand(0, count($image_array) - 1);
			echo $image_array[$random_number];
		} else {
			if (count($image_array) < intval($RandomPickup)) {
				$RandomPickup = count($image_array);
			}
			echo $image_array[intval($RandomPickup) - 1];
		}
	}
?>
</body>
